...
e $p(\mathbf{x} \mid c_k)$, esses valores são obtidos por estimadores de máxima
verossimilhança.
...
bom estimador de probabilidade das classes~\cite{schutze08}, Naïve Bayes se
...
função custo baseada na distância entre os dados e o centro de massa das classes.
...
aplicável à problemas não-lineares e ganhando assim alta adoção.
...
representações densas, passaram a ser utilizados classificadores não-lineares que
visam adicionar alguma forma de contexto ao processo.
...
A forma da rede convolucional atacar o problema da correlação espacial da
...
podendo cada passo ser uma palavra.
